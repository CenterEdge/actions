...
- If the run is triggered by a release tag, the version will be the tag name with any specified prefix removed. For example, if the tag is `v1.0.0` and the prefix is `v`, the version will be `1.0.0`.
...
- `tag-prefix` (optional): If your release tags have a prefix, such as `release/` or `v`, you can specify it here to ensure the version is calculated correctly. For example, if your tags are like `release/v1.0.0`, you would set this input to `release/v`.
